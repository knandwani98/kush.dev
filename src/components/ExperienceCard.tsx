...
export const ExperienceCard = (props: {
  ...
  const isDescriptionVisible = data?.description && data?.description?.length > 0;

  return (
    <div
      key={data.title}
      className={cn(
        "relative rounded-lg transition-all duration-300 px-2 sm:px-8 py-4",
        isDescriptionVisible ? "hover:bg-accent/20" : ""
      )}
    >
      {/* LINE */}
      {isTimeLineVisible && (
      ...
      <div className="flex justify-start items-start gap-2 sm:gap-4">
        ...
        <div className="w-full">
          ...
          <div className="flex items-centre justify-between gap-2">
            <h2 className="text-base font-semibold italic flex items-center justify-start gap-4">
      ...
      {isDescriptionVisible && (
        <div
          className={cn(
            "overflow-hidden transition-all duration-500 ease-in-out ml-8",
            isCollapsed ? "max-h-0 opacity-0" : "max-h-[1000px] opacity-100"
          )}
        >
          <ul className="mt-4 px-2 sm:px-4">
